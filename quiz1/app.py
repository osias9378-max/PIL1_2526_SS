...
@app.route("/sujet-page")
def sujet_page():
    return render_template("sujet_page.html")


@app.route("/generer-sujet",methods=["POST"])
def generer_sujet():
    data_recue = request.get_json()
    sujet = data_recue.get("sujet", "")

    if not sujet or len(sujet.strip()) == 0:
        return jsonify({"error": "Sujet vide"}), 400

    quiz = genere_question(sujet,mode="sujet")
    return jsonify(quiz)


def genere_question(text,mode="fichier"):
